...
public sealed partial class ShiftMapperGenerator
{
    ...
    internal static string SharedPackDeclarations(Compilation compilation, CancellationToken cancellationToken)
    {
        ...
        if (registrations.SharedPacks.Count == 0)
            return string.Empty;
        ...
        sb.AppendLine("// ShiftMapper declaration metadata. Do not edit.");
        sb.AppendLine("//");
        sb.AppendLine("// These packs were SHARED by a registration in this project — o.ShareConversions<T>() — with");
        sb.AppendLine("// every project that references it. The generator compiling such a project reads this and");
        sb.AppendLine("// gives the pack to every mapper it registers, as if o.AddConversions<T>() had been written");
        sb.AppendLine("// at the end of each of its AddShiftMapper calls.");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
